...
> **Status**: Draft v3.1 — 2026-06-09  
> **Target audience**: 小学 1–5 年级（约 6–11 岁）  
> **Supersedes**: Draft v3 (2026-05-12). v3.1 增补 §10.3 食物与护盾（v0 发布后首个玩法扩展）。
...
看数字 / 称号 → 判断追谁躲谁 →（可选）开 Boost 抢位或逃生
        ↓
（可选）吃地上食物 → 获得护盾层数（最多 4 层）
        ↓
撞小鱼 → 吃 → size 涨（rostered）或仅 size 涨（CatchUp，无 bonus）
        ↓
被大鱼咬 → 有护盾则存活；脱离威胁范围后消耗 1 层护盾
        ↓
...
| Shield | `SHIELD xN`；格挡中显示 `BLOCKING`；鱼身 ForceField 气泡（§10.3） |
...
### 10.3 食物与护盾（v0.1，2026-06）

| 项 | 规则 |
|----|------|
| 食物来源 | 关卡 `Playing` 阶段，地图上周期性刷新 **单个** 可拾取食物 |
| 刷新 | 每 **20 秒** 检查：若地上 **无** 食物则生成一个；已有未拾取的食物 **不移动**；被玩家吃掉后 **20 秒** 再生成 |
| 拾取者 | **仅人类玩家**；AI 经过食物 **无效果**，食物 **不消失** |
| 拾取奖励 | **+1 护盾层**；**不**增加 `size` / `bonus` |
| 层数上限 | **4 层**；已满 4 层时再碰到食物 → **不拾取**，食物保留 |
| 护盾效果 | 玩家被 **严格更大** 的 AI 或玩家咬住时：**不死亡**；攻击者 **无** `size` / `bonus` 收益 |
| 消耗时机 | **脱离** 所有更大威胁的碰撞范围 **之后** 才消耗 **1 层**（一次遭遇只扣 1 层，多条大鱼同时压住也只扣 1 层） |
| 0 层时 | 与 §5.2 正常死亡规则相同 |
| 与 spawn 保护 | 复活后 3 秒 spawn 保护仍优先；保护期内 AI 不能杀玩家，**不**触发护盾消耗逻辑 |
| 与 §5.5 debuff | 独立；护盾挡下攻击 **不** 给攻击者奖励，与 debuff 语义一致 |
| 死亡 | 玩家死亡并复活后 → **护盾清零** |
| 过关 | 活着通关 → **护盾保留** 到下一关 |
| Session 失败 | 全员 reset → **护盾清零** |
| UI | HUD 显示 `SHIELD xN`；格挡中可显示 `BLOCKING`；玩家鱼外圈 ForceField 气泡随层数变化 |
...
皮肤、DataStore 排行、任务、私人服、**地图障碍物 / 小迷宫**（用户已记录为后续功能）等。
...
| N9 人数上限 | playtest |
| 食物刷新语义 | 无食物才生成；已存在则不刷新位置 — **已定** |
| 护盾消耗语义 | 脱离威胁范围后扣 1 层；一次遭遇只扣 1 层 — **已定** |
| 护盾满层拾取 | 不拾取、食物保留 — **已定** |

**仍须在实现时拍板的细项（非阻塞）**：`LevelParticipants` 快照的 **精确帧**；`CatchUp` 是否在 `size ≥ target` 时显示类 frozen 纯 UI；PC Boost 默认键位（当前实现为 `F`）。
